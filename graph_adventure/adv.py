...
from utils import Graph, Stack, Queue
...
# Moves are used to describe movement
# A move consists of the starting room and a direction
# Examle: {origin: 0, direction:'n'} - from room 0 move north
class Move:
...
def find_shortest_path(graph, starting_room):
	...
	while qq.size() > 0:
		...
		path = qq.dequeue()
		move = path[-1]
		print(f'    move origin:{move.origin}')
		print(f'    move dir:{move.dir}')
		room = path[-1].origin
		if move.dir:
			room = graph[room][move.dir]
		print('    currRoom: ' + str(room))
...
# moveQueue = Queue() # queue of moves
stack.push(Move(player.currentRoom.id, None))
count = 1
while stack.size() > 0:
	print(f'<---------dft loop: {count}--------->')

	move = stack.pop()
	prevRoom = move.origin
	move.dir and player.travel(move.dir)
	move.dir and traversalPath.append(move.dir)
	move.dir and print("Move( Origin: " + str(move.origin) + " | Dir: " + move.dir)
	currRoom = player.currentRoom.id
	print('prevRoom: ' + str(prevRoom))
	print('currRoom: ' + str(currRoom))
	...
	if currRoom not in visited.keys():
		...
		# populates exits with ? or roomid of last move
		for exit in exits:
			if exit not in visited[currRoom]:
				if move.dir and exit == move.reverse():
					...
					visited[currRoom][exit] = '?'
					visited[currRoom]['?'] += 1
					new_move = Move(currRoom, exit)
					stack.push(new_move)

				# print("Ray( Origin: " + str(new_move.origin) + " | Dir: " + new_move.dir)
				# print('exit: ' + exit)

	# if no new rooms found
	if visited[currRoom]['?'] == 0:
		path, new_starting_move = find_shortest_path(visited, Move(currRoom, None))
		# if count == 4:
			# break

		if len(path) > 0:
			for move in path:
				print(f'path( ori: {move.origin}, dir: {move.dir}')
				player.travel(move.dir)
				traversalPath.append(move.dir)
		# print(f'new starting( ori: {new_starting_move.origin}, dir: {new_starting_move.dir}')

		stack = Stack()
		new_starting_move and stack.push(new_starting_move)


	# print(visited)
	print(f"<----------------dft{count} END---------------->")
	count +=1
