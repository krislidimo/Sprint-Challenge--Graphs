...
#       |         |         |                      #
#       |         |         |                      #
#      016--015--001--012--013       00x           #
#                 |                   |            #
#                 |                   |            #
#      008--007--000--003--004--00x--00x--00x      #
#       |         |         |    |                 #
#       |         |         |    |                 #
#      009       005       00x  00x                #
#       |         |                                #
...
# breath first traversal to find the closest '?'
...
while stack.size() > 0:
	...
	# if no new rooms found and unknown rooms exist
	if visited[currRoom]['?'] == 0 or unknownRooms > 0:
		path, new_starting_move = find_shortest_path(visited, Move(currRoom, None))

		# returns player to the closest unknown room
		if path and new_starting_move:
			for move in path:
				player.travel(move.dir)
				traversalPath.append(move.dir)

		stack = Stack()
		if new_starting_move:
			stack.push(new_starting_move)
